...
def ingest_menu():
    ...
    source_id = Prompt.ask(
        "Enter source ID (e.g. url or title)", default="manual-input"
    )
    with console.status("[bold green]Ingesting and extracting knowledge..."):
        try:
...
def graphrag_menu():
    ...
    with console.status("[bold magenta]GraphRAG reasoning in progress..."):
        try:
            res = client.graphrag(GraphRAGRequest(natural_language_query=query))
            console.print(
                Panel(res.answer, title="GraphRAG Answer", border_style="magenta")
            )
...
def validate_menu():
    with console.status("[bold yellow]Validating graph..."):
        try:
            res = client.validate()
