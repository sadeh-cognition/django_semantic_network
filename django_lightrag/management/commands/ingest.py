...
@click.command()
@click.option('--file', required=True, type=click.Path(exists=True), help='Path to text file')
...
def command(file, source_id):
    ...
    try:
        ...
        if log.status == "success":
            click.secho(f"Success! Extracted {log.concepts_extracted} concepts and {log.relations_extracted} relations.", fg="green")
        else:
